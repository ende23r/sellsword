...
describe('parseSheetStats', () => {
  it('parses all fields from a complete cell set', () => {
    const stats = parseSheetStats(makeCells(), INFANTRY_ROWS, CAVALRY_ROWS);
...
describe('derived detachment totals', () => {
  ...
  it('isCavalryOnly is false when the army has infantry or any wagons', () => {
    expect(isCavalryOnly(parseSheetStats(makeCells(), INFANTRY_ROWS, CAVALRY_ROWS))).toBe(false);
...
describe('statWriteData', () => {
  const stats = parseSheetStats(makeCells({ hex: '3,-2', forced_march: 1 }), INFANTRY_ROWS, CAVALRY_ROWS);
