...
Army stats live in Google Sheets, not the DB. The contract between the bot and a sheet is the **named ranges** listed in `STAT_RANGE_NAMES` (`src/lib/sheets.ts`): 12 single-cell scalars named exactly after the `ArmySheetStats` keys (`morale`, `hex`, …) plus `detachments`, a multi-row range over the detachments table (columns `name | size | notes | multiplier | strength | wagons`, header excluded, blank rows ignored). GMs define ranges once via Data → Named ranges and can otherwise lay out their sheets however they like — named ranges follow their cell when rows/columns move. `npm run check-sheet` is the GM-facing validator.

Detachments are the fundamental troop representation; there are no aggregate infantry/cavalry counts. `multiplier` (daily supplies eaten per soldier: infantry 1, cavalry 10) and `strength` are explicit per row so GMs can invent custom detachment types. The bot derives totals via `supplyUpkeep`/`totalStrength`/`totalWagons`/`isCavalryOnly` (`src/lib/sheets.ts`). The bot only ever writes the ranges in `statWriteData` (`morale`, `supplies`, `coin`, `goods`, `hex`, `stance`, `forced_march`, `night_march`); everything else — detachments, `noncombatants`, `resting_morale`, `max_morale`, `scouting_range` — is GM-owned and read-only to the bot. Battle casualties are therefore reported by `/battle` but applied to detachment rows manually by the GM.
