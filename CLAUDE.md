...
Discord commands (in `src/commands/`): `/queue`, `/unqueue`, `/recruit`, `/commission`, `/retire`, `/move`, `/forage`, `/sell`, `/siege`, `/pace`, `/stance`, `/transfer`, `/message`, `/gmping`, `/map`, `/list-armies`, `/drop-army`, `/drop-message`, `/roll`, `/ticknow` (admin), `/battle` (admin)

Army stats live in Google Sheets, not the DB. The contract between the bot and a sheet is the **named ranges** listed in `STAT_RANGE_NAMES` (`src/lib/sheets.ts`): 11 single-cell scalars named exactly after the `ArmySheetStats` keys (`morale`, `hex`, …) plus three table ranges — `infantry_detachments` and `cavalry_detachments` (columns `name | size | notes | multiplier | strength | wagons`) and `goods` (columns `name | count`, counts on the same scale as supplies). Table ranges cover data rows only (header excluded); a row registers only when its size/count is a positive number, so template rows may prefill default multipliers and strength formulas. GMs define ranges once via Data → Named ranges and can otherwise lay out their sheets however they like — named ranges follow their cell when rows/columns move. `npm run check-sheet` is the GM-facing validator.

Detachments are the fundamental troop representation; there are no aggregate infantry/cavalry counts. Infantry and cavalry are separate tables because the rules treat them differently. `multiplier` (daily supplies eaten per soldier; defaults to 1 in the infantry table, 10 in the cavalry table) and `strength` are explicit per row so GMs can invent custom detachment types. The bot derives totals via `supplyUpkeep`/`totalStrength`/`totalWagons`/`isCavalryOnly` (`src/lib/sheets.ts`); an army is cavalry-only when it has cavalry detachments, no infantry detachments, and no wagons. The bot only ever writes the ranges in `statWriteData` (`morale`, `supplies`, `coin`, `hex`, `stance`, `forced_march`, `night_march`); everything else — the three tables, `noncombatants`, `resting_morale`, `max_morale`, `scouting_range` — is GM-owned and read-only to the bot, with one exception: resolving a sell order rewrites the `goods` table (clear + rewrite via `writeGoods`). Battle casualties are reported by `/battle` but applied to detachment rows manually by the GM, and `/transfer` moves only supplies and coin (named goods move via GM sheet edits or `/sell`).

**Markets:** the admin sheet has a `Demands` tab (headers `Hex | Good | Price | Volume`) that the GM edits live — each row is a market for one good in one hex: `Hex` is `q,r`, `Price` is coin per unit, `Volume` is the units/day the market absorbs. `/sell` queues a sell order — a series of one-day rests processed at the night tick (`processSellOrders`): each day the army sells goods matching demands in its hex, a demand's daily volume is split evenly among all armies selling that good there, and the order completes when no marketable goods remain (unmatched goods stay in inventory). Bad Demands rows are skipped with warnings in the tick log; if the tab is unreadable, sell processing is skipped for the tick rather than cancelling orders.

**Sieges:** the admin sheet has a `Strongholds` tab (headers `Hex | Name | Type | Garrison | Controlled By | Threshold | Siege Start | Last Roll`) that is the live source of truth for mutable stronghold state; the DB `strongholds` table is a one-way mirror (`syncStrongholdsToDb`) so map rendering keeps working. The bot writes only the last three columns (F:H); the rest are GM-owned. `/siege` queues a continuous siege order (target hex frozen in parameters). At the night tick (`processSieges`): a new siege sets `Siege Start`; every 7 days since the later of `Siege Start`/`Last Roll` the threshold drops by 1 (the default −1/week — the GM edits the Threshold cell live for disease/resupply/sortie modifiers) and 2d6 is rolled — over the threshold opens the gates (bot announces with weeks-under-siege for the 1d6−X capture rule; GM applies capture and `Controlled By` manually, per the `/battle` precedent). Sieges change state only on positive evidence: an army with no stats this tick still counts as besieging, and an unreadable tab skips siege processing entirely. At the morning tick (`recoverThresholds`), unbesieged strongholds recover threshold +1/day up to the type default (town 10 / city 15 / fortress 20); GM-set values above the default are left alone.
